...
function displayAllLanguages(guideLng: string[], touristLng: string[]): string{
    let result: string = "<h1>Gidai</h1>";
    guideLng.forEach((lng: string) => {
        result +=` ${lng} <br> `; });
    result += "<h1>Turistai</h1>";
    touristLng.forEach((lng: string) => {
        result +=` ${lng} <br> `; });
    return result;
}
